 ...
 *   Nuk4sd --new work --path /data/w --protected
...
extern int rust_cgroup_apply(const char *cgroup_name, unsigned long long pid, long long memory_limit_mb,
                             unsigned long long cpu_shares);
extern int rust_cgroup_remove(const char *cgroup_name);
...
#ifdef __linux__
static int _rm_rf_cb(const char *path, const struct stat *sb, int typeflag, struct FTW *ftwbuf) {
    (void)sb;
    (void)ftwbuf;
 ...
 * para construir os argumentos de execução de forma segura. */
...
enum {
    ...
    OPT_NO_FUSE,
    /* Modo sandbox aninhado: fronteira externa mantida, app pode montar
     * o próprio sandbox interno (setuid/seccomp/namespace próprios) */
    OPT_NESTED_SANDBOX = 120,
    OPT_MOZ_COMPAT,
    /* Inspeciona isolamento de um PID em execução */
    OPT_HEALTH,
    /* Filtro de rede nftables (--nfilter) */
    OPT_NFILTER,
    /* Supervisor de UUID do sandbox (--uuid) */
    OPT_UUID,
};

static const struct option long_options[] = {{"vault", required_argument, NULL, OPT_VAULT},
                                             ...
                                             {"unshare-cgroup", no_argument, NULL, OPT_UNSHARE_CGROUP},
                                             ...
                                             {"allow", required_argument, NULL, OPT_ALLOW},
                                             {"seccomp-strict", no_argument, NULL, OPT_SECCOMP_STRICT},
                                             {"allow-clone3", no_argument, NULL, OPT_ALLOW_CLONE3},
                                             {"friendly-sandbox", no_argument, NULL, OPT_FRIENDLY_SANDBOX},
                                             {"permissive", no_argument, NULL, OPT_PERMISSIVE_SANDBOX},

                                             {"nested-sandbox", no_argument, NULL, OPT_NESTED_SANDBOX},
                                             {"moz-compat", no_argument, NULL, OPT_MOZ_COMPAT},
                                             {"no-fuse", no_argument, NULL, OPT_NO_FUSE},
                                             {"health", required_argument, NULL, OPT_HEALTH},
                                             /* rede: filtro nftables */
                                             {"nfilter", required_argument, NULL, OPT_NFILTER},
                                             /* supervisor uuid do sandbox */
...
static void print_ok(const char *msg) {
    printf("\033[32mœ” %s\033[0m\n", msg);
}
static void print_err(const char *msg) {
    fprintf(stderr, "\033[31mœ– %s\033[0m\n", msg);
}
static void print_warn(const char *msg) {
    fprintf(stderr, "\033[33mš  %s\033[0m\n", msg);
}
...
static char *read_password_silent(const char *prompt) {
    ...
    nw = old;
    nw.c_lflag &= ~ECHO;
    tcsetattr(STDINFO, TCSANOW, &nw);
    memset(buf, 0, sizeof(buf));
    if (fgets(buf, sizeof(buf), stdin))
        buf[strcspn(buf, "\n")] = '\0';
    tcsetattr(STDINFO, TCSANOW, &old);
    fprintf(stderr, "\n");
    return buf;
}
...
static void print_help(void) {
    printf("\nNuk4sd  hardened vault & isolation engine\n"
           ...
           "  --image <url|alias|file:///abs/rootfs.tar.gz>  importa rootfs explícito antes do run\n\n"
           ...
           "    --net veth           isolated veth pair + NAT  jail gets its own\n"
           "                         network namespace with a dedicated interface\n"
           "                         (default: gw=" VETH_IP_GW ", jail=" VETH_IP_JAIL VETH_CIDR ")\n"
           "    --nfilter <name>     aplica filtro nftables com a lista de IPs permitidos\n\n"
           "  Supervisor / Identificação:\n"
           "    --uuid               gera UUID dinmico e monitora o processo sandbox\n\n"
           "  Display:\n"
           ...
           "    --unshare-cgroup     isolate cgroup namespace and apply requested limits\n"
           ...
           "  Nuk4sd --new work --path /data/work --protected\n"
           "  Nuk4sd --vault 3 --protect-delete --protect-write\n"
...
static void load_profile(CliConfig *cfg, const char *path) {
    FILE *f = fopen(path, "r");
    if (!f) {
        fprintf(stderr, "š  profile '%s' not found\n", path);
        return;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (!strcmp(p, "--no-net"))
            cfg->iso_no_net = true;
        else if (!strcmp(p, "--wayland"))
            cfg->iso_wayland = true;
        else if (!strcmp(p, "--x11"))
            cfg->iso_x11 = true;
        else if (!strcmp(p, "--ro-home"))
            cfg->iso_ro_home = true;
        else if (!strcmp(p, "--rw-home"))
            cfg->iso_rw_home = true;
        else if (!strcmp(p, "--no-dbus"))
            cfg->iso_no_dbus = true;
        else if (!strcmp(p, "--tmp-home"))
            cfg->iso_tmp_home = true;
        else if (!strcmp(p, "--audit"))
            cfg->iso_audit = true;
        else if (!strcmp(p, "--no-proc"))
            cfg->iso_no_proc = true;
        else if (!strcmp(p, "--new-session"))
            cfg->iso_new_session = true;
        else if (!strcmp(p, "--unshare-ipc"))
            cfg->iso_unshare_ipc = true;
        else if (!strcmp(p, "--unshare-uts"))
            cfg->iso_unshare_uts = true;
        else if (!strcmp(p, "--unshare-cgroup"))
            ...
            cfg->binds[cfg->bind_count++].type = BINFO;
 ...
 *
 *  Necessário para --ro/--rw: mkdir() simples falha com ENOENT se qualque
...
static void cli_expand_tilde(const char *in, char *out, size_t out_sz) {
    ...
    if (!home) {
        ...
        fprintf(stderr, "š  cli_expand_tilde: HOME não definido, mantendo '%s' literal\n", in);
        snprintf(out, out_sz, "%s", in);
        return;
    }
...
static bool bind_path_is_safe(const char *vault_path, const char *dst) {
    ...
    if (!realpath(vault_path, resolved_vault))
        return false;
    ...
    while (!realpath(probe, resolved_probe)) {
        ...
        if (!slash || slash == probe)
            return false; /* chegou na raiz sem achar nada existente */
        *slash = '\0';
    }
...
static int parse_flags(int argc, char **argv, CliConfig *cfg) {
    ...
    /* CRÃTICO: optind é estático/global na libc e persiste entre chamadas
    ...
    while ((opt = getopt_long(argc, argv, "qk", long_options, &opt_index)) != -1) {
        switch (opt) {
        case OPT_VAULT:
            cfg->vault_id = (int32_t)atoi(optarg);
            break;
        case OPT_LS:
            cfg->op_ls = true;
            break;
        ...
        case OPT_UMOUNT:
            cfg->op_umount = true;
            break;
        case OPT_MOUNT_EXPORT:
            cfg->op_mount_export = true;
            break;
        case OPT_EXPORT:
            cfg->op_export = true;
            break;
        case OPT_FILE:
            cfg->export_file = optarg;
            break;
        case OPT_DEST:
            cfg->export_dest = optarg;
            break;
        case OPT_RM:
            cfg->op_rm = true;
            break;
        ...
        case OPT_HOURS: {
            ...
            if (!dash) {
                print_err("--hours: use format 9-18");
                return -1;
            }
        ...
        case OPT_PATH:
            ...
            break;
        case OPT_PROTECTED:
            cfg->protected_vault = true;
            break;
        /* WORM */
        case OPT_WORM_STATUS:
            cfg->op_worm_status = true;
            break;
        case OPT_PROTECT_DELETE:
            cfg->worm_set |= WORM_PROTECT_DELETE;
            break;
        case OPT_PROTECT_RENAME:
            cfg->worm_set |= WORM_PROTECT_RENAME;
            break;
        case OPT_PROTECT_WRITE:
            cfg->worm_set |= WORM_PROTECT_WRITE;
            break;
        case OPT_PROTECT_READ:
            cfg->worm_set |= WORM_PROTECT_READ;
            break;
        case OPT_PROTECTED_SCAN:
            cfg->worm_protected_scan = true;
            break;
        case OPT_CLEAR_DELETE:
            cfg->worm_clear |= WORM_PROTECT_DELETE;
            break;
        case OPT_CLEAR_RENAME:
            cfg->worm_clear |= WORM_PROTECT_RENAME;
            break;
        case OPT_CLEAR_WRITE:
            cfg->worm_clear |= WORM_PROTECT_WRITE;
            break;
        case OPT_CLEAR_READ:
            cfg->worm_clear |= WORM_PROTECT_READ;
            break;
        case OPT_WHITE_LIST:
        ...
        case OPT_NO_PROC:
            cfg->iso_no_proc = true;
            break;
        case OPT_NEW_SESSION:
            cfg->iso_new_session = true;
            break;
        case OPT_UNSHARE_IPC:
            cfg->iso_unshare_ipc = true;
            break;
        case OPT_UNSHARE_UTS:
            cfg->iso_unshare_uts = true;
            break;
        case OPT_UNSHARE_CGROUP:
            cfg->iso_unshare_cgroup = true;
            break;
        case OPT_HOSTNAME:
            cfg->iso_hostname = optarg;
            break;
        case OPT_PROFILE:
            cfg->iso_profile = optarg;
            break;
        /* desktop runtime */
        case OPT_AUDIO:
            cfg->iso_audio = true;
        ...
        case OPT_NO_SECCOMP:
            ...
            break;
        case OPT_CHROOT:
            cfg->iso_use_chroot = true;
            break;
        case OPT_DISPLAY_OPT:
            cfg->iso_display = optarg;
            break;
        case OPT_WAYLAND_DISPLAY:
            cfg->iso_wayland_disp = optarg;
            break;
        case OPT_DBUS:
            ...
            return -1;
        case OPT_DEV:
            if (!strcmp(optarg, "minimal") || !strcmp(optarg, "1"))
                cfg->iso_dev_level = 1;
            else if (!strcmp(optarg, "standard") || !strcmp(optarg, "2"))
                cfg->iso_dev_level = 2;
            else {
                print_err("--dev: use 'minimal', 'standard', '1' or '2'");
                return -1;
            }
            break;
        ...
        /* rede */
        case OPT_NET: {
            if (strcmp(optarg, "veth") == 0) {
                cfg->iso_net_veth = true;
                /* Defaults */
                if (!cfg->iso_net_veth_ip)
                    cfg->iso_net_veth_ip = VETH_IP_JAIL;
                if (!cfg->iso_net_veth_gw)
                    cfg->iso_net_veth_gw = VETH_IP_GW;
                if (!cfg->iso_net_veth_name)
                    cfg->iso_net_veth_name = "nuk4sd";
            } else {
                fprintf(stderr, "error: --net: modo '%s' desconhecido. Use 'veth'.\n", optarg);
                return -1;
            }
            break;
        }
        ...
        case OPT_HELP:
            cfg->op_help = true;
            break;
        case OPT_SECCOMP_STRICT:
            /* O novo modelo já é strict por padrão. Mantido para retrocompatibilidade. */
            break;
        case OPT_ALLOW_CLONE3:
            cfg->seccomp_allow |= SALLOW_CLONE3;
            break;
        case OPT_FRIENDLY_SANDBOX:
            cfg->seccomp_allow |= (SALLOW_FSYNC | SALLOW_RENAMEAT2);
            break;
        case OPT_PERMISSIVE_SANDBOX:
            cfg->seccomp_allow |= (SALLOW_CHROOT | SALLOW_SETUID | SALLOW_MOUNT);
            break;
        case OPT_ALLOW:
            if (!strcmp(optarg, "socket"))
                cfg->seccomp_allow |= SALLOW_SOCKET;
            else if (!strcmp(optarg, "bind"))
                cfg->seccomp_allow |= SALLOW_BIND;
            else if (!strcmp(optarg, "listen"))
                cfg->seccomp_allow |= SALLOW_LISTEN;
            else if (!strcmp(optarg, "accept"))
                cfg->seccomp_allow |= SALLOW_ACCEPT;
            else if (!strcmp(optarg, "connect"))
                cfg->seccomp_allow |= SALLOW_CONNECT;
            else if (!strcmp(optarg, "sendto"))
                cfg->seccomp_allow |= SALLOW_SENDTO;
            else if (!strcmp(optarg, "recvfrom"))
                cfg->seccomp_allow |= SALLOW_RECVFROM;
            else if (!strcmp(optarg, "sendmsg"))
                cfg->seccomp_allow |= SALLOW_SENDMSG;
            else if (!strcmp(optarg, "recvmsg"))
                cfg->seccomp_allow |= SALLOW_RECVMSG;
            else if (!strcmp(optarg, "socketpair"))
                cfg->seccomp_allow |= SALLOW_SOCKETPAIR;
            else if (!strcmp(optarg, "getsockopt"))
                cfg->seccomp_allow |= SALLOW_GETSOCKOPT;
            else if (!strcmp(optarg, "setsockopt"))
                cfg->seccomp_allow |= SALLOW_SETSOCKOPT;
            else if (!strcmp(optarg, "shmget"))
                cfg->seccomp_allow |= SALLOW_SHMGET;
            else if (!strcmp(optarg, "shmat"))
                cfg->seccomp_allow |= SALLOW_SHMAT;
            else if (!strcmp(optarg, "shmctl"))
                cfg->seccomp_allow |= SALLOW_SHMCTL;
            else if (!strcmp(optarg, "memfd"))
                cfg->seccomp_allow |= SALLOW_MEMFD;
            else if (!strcmp(optarg, "clone3"))
                cfg->seccomp_allow |= SALLOW_CLONE3;
            else if (!strcmp(optarg, "futex"))
                cfg->seccomp_allow |= SALLOW_FUTEX;
            else if (!strcmp(optarg, "fsync"))
                cfg->seccomp_allow |= SALLOW_FSYNC;
            else if (!strcmp(optarg, "renameat2"))
                cfg->seccomp_allow |= SALLOW_RENAMEAT2;
            else if (!strcmp(optarg, "inotify"))
                cfg->seccomp_allow |= SALLOW_INOTIFY;
            else if (!strcmp(optarg, "splice"))
                cfg->seccomp_allow |= SALLOW_SPLICE;
            else if (!strcmp(optarg, "timerfd"))
                cfg->seccomp_allow |= SALLOW_TIMERFD;
            else if (!strcmp(optarg, "signalfd"))
                cfg->seccomp_allow |= SALLOW_SIGNALFD;
            else if (!strcmp(optarg, "userfaultfd"))
                cfg->seccomp_allow |= SALLOW_USERFAULTFD;
            else if (!strcmp(optarg, "ptrace"))
                cfg->seccomp_allow |= SALLOW_PTRACE;
            else if (!strcmp(optarg, "bpf"))
                cfg->seccomp_allow |= SALLOW_BPF;
            else if (!strcmp(optarg, "mount"))
                cfg->seccomp_allow |= SALLOW_MOUNT;
            else if (!strcmp(optarg, "chroot"))
                cfg->seccomp_allow |= SALLOW_CHROOT;
            else if (!strcmp(optarg, "setuid"))
                cfg->seccomp_allow |= SALLOW_SETUID;
            else {
                fprintf(stderr, "unknown --allow syscall group: %s\n", optarg);
                exit(1);
            }
            break;

        case OPT_NESTED_SANDBOX:
            cfg->nested_sandbox = true;
            break;
        case OPT_MOZ_COMPAT:
            cfg->moz_compat = true;
            break;
        ...
        /* --nfilter <jail_name>: ativa filtro nftables para o jail */
        case OPT_NFILTER:
            cfg->iso_nfilter_jail = optarg;
            break;
        /* --uuid: habilita supervisor UUID para o processo sandbox */
        case OPT_UUID:
            cfg->iso_uuid = true;
            break;
        /* bind mounts */
 ...
 *
 * Esta função é a ÃšNICA fonte de verdade pra isso no arquivo  antes,
...
static uid_t resolve_real_uid(void) {
    ...
    if (sudo_uid_s && *sudo_uid_s) {
        ...
        if (errno == 0 && end != sudo_uid_s && *end == '\0' && parsed >= 0 && parsed <= 65535)
...
static gid_t resolve_real_gid(void) {
    ...
    if (sudo_gid_s && *sudo_gid_s) {
        ...
        if (errno == 0 && end != sudo_gid_s && *end == '\0' && parsed >= 0 && parsed <= 65535)
...
static int run_isolated(CliConfig *cfg, char *vault_path) {
    ...
    /* cfg->image_url_allocated só é definido após uma importação de imagem
     * bem-sucedida. Nesse caso o diretório extraído deve virar '/', em vez de
     * ser apenas montado em /vault dentro de um jail vazio. */
    const bool imported_rootfs = cfg->image_url_allocated;
    ...
    if (mkdtemp(jail_root) == NULL) {
        /* Fallback para /var/tmp se /tmp falhar devido a permissões estritas ou restrições de VFS */
        snprintf(jail_root, sizeof(jail_root), "/var/tmp/Nuk4sd-jail-XXXXXX");
        if (mkdtemp(jail_root) == NULL) {
            perror("[RUN] mkdtemp jail_root (falhou em /tmp e /var/tmp)");
            return -1;
        }
    }

    /* Assegura permissões 0755 no ponto de montagem antes de entrar no mount/user namespace */
    chmod(jail_root, 0755);
    ...
    /* O pai ainda possui as credenciais do host; normaliza o owner do
     * diretório privado antes do filho entrar no user namespace. Isso é
     * necessário quando o CLI foi iniciado via sudo, porque o filho depois
     * executará como real_uid/real_gid e criará o scaffolding do rootfs. */
    struct stat jail_st;
    if (stat(jail_root, &jail_st) != 0) {
        fprintf(stderr, "[RUN][FATAL] stat do jail privado '%s' falhou: %s\n", jail_root, strerror(errno));
        rmdir(jail_root);
        return -1;
    }
    if (jail_st.st_uid != real_uid || jail_st.st_gid != real_gid) {
        if (chown(jail_root, real_uid, real_gid) != 0) {
            fprintf(stderr, "[RUN][FATAL] chown do jail privado '%s' falhou: %s\n", jail_root, strerror(errno));
            rmdir(jail_root);
            return -1;
        }
    }
    ...
    if (cfg->iso_audit) {
        ...
        fprintf(stderr, "[audit] no-net=%d  wayland=%d  x11=%d  ro-home=%d\n", cfg->iso_no_net, cfg->iso_wayland,
                cfg->iso_x11, cfg->iso_ro_home);
        fprintf(stderr, "[audit] no-dbus=%d  tmp-home=%d  no-proc=%d\n", cfg->iso_no_dbus, cfg->iso_tmp_home,
                cfg->iso_no_proc);
        fprintf(stderr, "[audit] unshare-ipc=%d  unshare-uts=%d  unshare-cgroup=%d  new-session=%d\n",
                cfg->iso_unshare_ipc, cfg->iso_unshare_uts, cfg->iso_unshare_cgroup, cfg->iso_new_session);
        for (int i = 0; i < cfg->bind_count; i++) {
            const char *t = cfg->binds[i].type == BINFO       — "ro" : cfg->binds[i].type == BIND_RW       — "rw" : "blacklist";
    ...
    /* Log massivo da configuração do sandbox */
    {
        ...
        cli_log_sandbox_config(cfg->run_exec, vault_path, cfg->iso_no_net, cfg->iso_wayland, cfg->iso_x11,
                               cfg->iso_no_dbus, cfg->iso_ro_home, cfg->iso_tmp_home, cfg->iso_no_proc,
                               cfg->iso_unshare_ipc, cfg->iso_unshare_uts, cfg->iso_new_session, cfg->iso_hostname,
    ...
    /* [Rede] Criar par veth e mover pro namespace do filho
     * Executado pelo PAI: precisa de CAP_NET_ADMIN no netns do HOST pra cria
     * o par veth e movê-lo pro netns do filho (via /proc/<pid>/ns/net).
     * Depois que veth1 entra no netns do filho, só o filho pode configurá-lo. */
    if (pid > 0 && cfg->iso_net_veth) {
        if (vsb_net_veth_setup(pid, cfg->iso_net_veth_ip, cfg->iso_net_veth_gw, cfg->iso_net_veth_name) != 0) {
            fprintf(stderr, "[RUN] WARN: veth setup failed  jail will have no network\n");
            /* Não-fatal: jail continua sem rede (igual --no-net) */
        }
    }
     ...
     *  */
    if (pid > 0) {
        char cgroup_name[96] = {0};
        int cgroup_applied = 0;
        ...
        if (vsb_write_uid_gid_map(pid, real_uid, real_gid) != 0) {
            ...
            if (rm_rf_safe(jail_root) != 0)
                fprintf(stderr, "[RUN] rm_rf_safe('%s') falhou: %s\n", jail_root, strerror(errno));
            return -1;
        }
        /* cgroup v2 é aplicado pelo pai, fora do namespace do filho. O filho
         * já criou o próprio namespace cgroup, então só enxerga a hierarquia
         * virtual; o controle efetivo permanece na hierarquia delegada do host. */
        if (cfg->iso_unshare_cgroup && cfg->iso_max_mem_gb > 0) {
            snprintf(cgroup_name, sizeof(cgroup_name), "nuk4sd-%ld-%d", (long)getpid(), (int)pid);
            long long memory_mb = (long long)cfg->iso_max_mem_gb * 1024LL;
            if (rust_cgroup_apply(cgroup_name, (unsigned long long)pid, memory_mb, 1024ULL) != 0) {
                fprintf(stderr, "[RUN][FATAL] cgroup v2 não pÃ´de ser aplicado  abortando sandbox.\n");
                kill(pid, SIGKILL);
                close(sync_pipe[1]);
                int status;
                waitpid(pid, &status, 0);
        ...
        waitpid(pid, &status, 0);
        if (cgroup_applied)
            rust_cgroup_remove(cgroup_name);
        vault_auth_pid_remove_ffi(pid);
        if (cfg->iso_net_veth)
            vsb_cleanup_veth(cfg->iso_net_veth_name);

        /* Limpa jail_root temporário (/tmp/Nuk4sd-jail-XXXXXX):
         * o scaffolding cria subdiretórios, portanto rmdir() não basta.
         * FTW_PHYS impede seguir symlinks durante a remoção. */
        if (umount2(jail_root, MNT_DETACH) != 0 && errno != EINVAL && errno != EPERM)
            fprintf(stderr, "[RUN] umount jail_root '%s': %s\n", jail_root, strerror(errno));
        if (rm_rf_safe(jail_root) != 0)
            fprintf(stderr, "[RUN] rm_rf_safe('%s') falhou: %s\n", jail_root, strerror(errno));

        if (WIFSIGNALED(status)) {
            fprintf(stderr,
                    "[RUN] process killed by signal %d "
    ...
    /* O mapa de identidade não altera automaticamente o UID inicial 0 do
     * processo. Transiciona para o UID/GID real mapeado antes de qualque
     * mount; sem isso, o payload aparece como overflowuid=65534 e libc
     * estática pode retornar EOVERFLOW em mkdtemp(). */
    if (vsb_enter_mapped_identity(real_uid, real_gid) != 0) {
        fprintf(stderr, "[RUN][FATAL] transição para UID/GID mapeado falhou  abortando sandbox.\n");
        _exit(1);
    }

    /* Uma imagem importada é a raiz explícita da execução. Primeiro isolamos
     * a árvore de mounts, tornamo-la privada e bind-montamos esse rootfs em
     * jail_root. Assim pivot_root() trocará para a imagem, não para um jail
     * vazio com a imagem escondida em /vault. */
    if (imported_rootfs) {
        if (unshare(CLONE_NEWNS) != 0) {
            fprintf(stderr, "[RUN] unshare CLONE_NEWNS para rootfs importado: %s\n", strerror(errno));
            _exit(1);
        }
        if (mount("none", "/", NULL, MS_REC | MS_PRIVATE, NULL) != 0) {
            fprintf(stderr, "[RUN] MS_PRIVATE antes do rootfs importado: %s\n", strerror(errno));
            _exit(1);
        }
        if (mount(vault_path_orig, vault_path, NULL, MS_BIND | MS_REC, NULL) != 0) {
            fprintf(stderr, "[RUN] bind rootfs importado '%s''%s': %s\n", vault_path_orig, vault_path, strerror(errno));
            _exit(1);
        }
    }

    /* Preparar o jail antes de CLONE_NEWPID é obrigatório: a preparação pode
     * consultar/copiar o busybox e, em instalações sem a dependência, aciona
     * um helper. Depois de CLONE_NEWPID, qualquer processo filho auxiliar pode
     * virar PID 1 temporário; quando termina, o namespace fica sem init e o
     * fork principal retorna ENOMEM. */
    ...
    /* --net veth SEMPRE cria network namespace isolado (mesmo sem --no-net).
     * A diferença: com --no-net o namespace fica VAZIO (sem interfaces);
     * com --net veth, injetamos um par veth isolado com NAT via host. */
    if (cfg->iso_no_net || cfg->iso_net_veth)
        ns_flags |= CLONE_NEWNET;
    if (cfg->iso_unshare_ipc)
        ns_flags |= CLONE_NEWIPC;
    if (cfg->iso_unshare_uts)
        ns_flags |= CLONE_NEWUTS;
#ifdef CLONE_NEWCGROUP
    if (cfg->iso_unshare_cgroup)
    ...
    /* [Rede] Configurar interface veth dentro do namespace
     * O par veth foi criado pelo PAI (que tem CAP_NET_ADMIN no netns do host)
     * e movido pro namespace do filho via /proc/<pid>/ns/net. Aqui, dentro do
     * namespace já isolado, ativamos a interface e configuramos IP + gateway. */
    if (cfg->iso_net_veth) {
        vsb_configure_veth_inside(cfg->iso_net_veth_ip, cfg->iso_net_veth_gw, cfg->iso_net_veth_name);
    }
    ...
    if (imported_rootfs) {
        fprintf(stderr, "[RUN] jail_root: '%s' (rootfs importado como raiz da sandbox)\n", jail_root);
    } else {
    ...
    /* A estrutura do jail já foi preparada antes de CLONE_NEWPID, para que
     * nenhum helper auxiliar seja criado antes do PID 1 definitivo. */
    ...
    pid_t ns_pid = fork();
    if (ns_pid < 0) {
        perror("[RUN] fork PID NS");
        _exit(1);
    }
    if (ns_pid > 0) {
        /* [SUPERVISOR UUID] Se --uuid foi passado, registra e monitora o UUID do sandbox */
        if (cfg->iso_uuid) {
            UuidArgs uargs = {.pid = ns_pid, .original_binary = cfg->run_exec, .pipe_fd = {-1, -1}};
            setup_uuid(uargs);
        }

        /* [NFILTER] Se --nfilter foi passado, aplica as regras do nftables para o jail */
        if (cfg->iso_nfilter_jail && cfg->iso_nfilter_jail[0] != '\0') {
            nfilterflag(cfg->iso_nfilter_jail, cfg->iso_net_veth_ip ? cfg->iso_net_veth_ip : VETH_IP_JAIL);
        }
    ...
    /* O /tmp precisa existir como tmpfs antes dos binds. Se o tmpfs fosse
     * montado somente após o pivot_root(), ele ocultaria qualquer bind
     * montado em /tmp (por exemplo, arquivos de trabalho de LibreOffice,
     * Chromium ou probes), fazendo o caminho desaparecer no processo filho.
     * O mount namespace já está privado neste ponto, portanto a montagem
     * continua confinada ao jail. */
    {
        int sz = cfg->iso_tmp_size_mb ? cfg->iso_tmp_size_mb : 64;
        /*
         * Em um user namespace com mapa de identidade, UID/GID 0 do
         * tmpfs não são necessariamente mapeados. Sem ownership explícito,
         * stat(2) dentro do jail expõe overflowuid/overflowgid (65534), e
         * libc estática pode retornar EOVERFLOW em mkdtemp(), apesar de
         * /tmp ser 01777 e gravável. Declarar o par mapeado preserva o
         * isolamento e mantém a ABI observável para aplicações estáticas.
         */
        char sz_opt[96];
        snprintf(sz_opt, sizeof(sz_opt), "uid=%u,gid=%u,mode=1777,size=%dm", (unsigned)real_uid, (unsigned)real_gid,
                 sz);
        char tmp_root[PATH_MAX];
        snprintf(tmp_root, sizeof(tmp_root), "%s/tmp", vault_path);
        if (cli_mkdir_p(tmp_root, 01777) != 0) {
            fprintf(stderr, "[RUN] mkdir tmpfs '%s': %s\n", tmp_root, strerror(errno));
        } else if (mount("tmpfs", tmp_root, "tmpfs", MS_NOSUID | MS_NODEV, sz_opt) != 0) {
            fprintf(stderr, "[RUN] mount /tmp pré-bind: %s\n", strerror(errno));
        }
    }
    ...
    for (int i = 0; i < cfg->bind_count; i++) {
        ...
        case BIND_RW: {
            ...
            if (mount(src, dst, NULL, MS_BIND | MS_REC, NULL) != 0) {
                ...
                mount(NULL, dst, NULL, MS_BIND | MS_REMOUNT | MS_NOSUID | MS_NODEV | MS_REC, NULL);
        ...
        case BIND_BLACKLIST:
            ...
            if (S_ISDIR(st.st_mode)) {
                if (mount("tmpfs", src, "tmpfs", MS_NOSUID | MS_NODEV | MS_RDONLY, "size=0") != 0) {
                    fprintf(stderr, "[RUN] --blacklist dir '%s': %s\n", src, strerror(errno));
                    cli_log_mount_event("tmpfs", src, "blacklist-dir", MS_NOSUID | MS_NODEV | MS_RDONLY, errno);
                } else {
                    cli_log_mount_event("tmpfs", src, "blacklist-dir", MS_NOSUID | MS_NODEV | MS_RDONLY, 0);
                }
            } else {
    ...
    if (cfg->iso_ro_home || cfg->iso_rw_home) {
        ...
        if (sudo_user && *sudo_user) {
            ...
            bool sudo_user_safe = (strlen(sudo_user) < 64 && strpbrk(sudo_user, "/.\\ :@!") == NULL);
            if (sudo_user_safe) {
                snprintf(home, sizeof(home), "/home/%s", sudo_user);
            } else {
                fprintf(stderr,
                        "[RUN] SUDO_USER cont\u00e9m caracteres inv\u00e1lidos  ignorando (possivel path traversal)\n");
        ...
        if (home[0] && stat(home, &st) == 0) {
            ...
            if (cli_mkdir_p(dst, 0755) == 0) {
                ...
                snprintf(nuk_fuse, sizeof(nuk_fuse), "%s%s/.local/share/Nuk4sd", vault_path, home);
                ...
                if (mount(home, dst, NULL, MS_BIND | MS_REC, NULL) == 0) {
                    if (cfg->iso_ro_home) {
                        ...
                        snprintf(nuk_mnt, sizeof(nuk_mnt), "%s%s/.local/share/Nuk4sd", vault_path, home);
                        if (cli_mkdir_p(nuk_mnt, 0700) == 0) {
                            char home_tmpfs_opts[96];
                            snprintf(home_tmpfs_opts, sizeof(home_tmpfs_opts), "uid=%u,gid=%u,mode=0700,size=4m",
                                     (unsigned)real_uid, (unsigned)real_gid);
                            mount("tmpfs", nuk_mnt, "tmpfs", MS_NOSUID | MS_NODEV, home_tmpfs_opts);
                        }
                    }
                } else {
     ...
     * tem o que o Nuk4sd faça sozinho; é preciso `sudo -E` ou configura
    ...
    if (cfg->iso_wayland) {
        ...
        if (stat(xdg, &ws) == 0) {
            if (mount(xdg, dst_xdg, NULL, MS_BIND | MS_REC, NULL) == 0)
                mount(NULL, dst_xdg, NULL, MS_BIND | MS_REMOUNT | MS_RDONLY | MS_REC, NULL);
        } else {
            fprintf(stderr,
    ...
    if (cfg->iso_x11) {
        ...
        if (stat(x11_src, &xs) == 0) {
            if (mount(x11_src, x11_dst, NULL, MS_BIND | MS_REC, NULL) == 0)
                mount(NULL, x11_dst, NULL, MS_BIND | MS_REMOUNT | MS_RDONLY | MS_REC, NULL);
        }
    ...
    if (!cfg->iso_no_proc) {
        ...
        mkdir(j_proc, 0555);
        mount("proc", j_proc, "proc", MS_NOSUID | MS_NOEXEC | MS_NODEV, NULL);
    }
     ...
     * qualquer forma. Montar tmpfs aqui só servia pra soterrar qualque
     * --ro/--rw que o usuário tenha aplicado sob /tmp antes deste ponto.
     *
     * LIMITACAO CONHECIDA: qualquer --ro/--rw/--blacklist cujo destino
     ...
     *   /etc/ld.so.conf.d        paths extras do dynamic linke
     ...
     * */
    if (gui_mode) {
        const char *host_dirs[] = {/* Ordem importa: /usr precisa vir antes de /usr/share/icons.
        ...
        for (int i = 0; host_dirs[i]; i++) {
             ...
             * do /usr, e o GTK morre com "endpoint desconectado" ao tenta
        ...
        const char *etc_files[] = {"/etc/ld.so.cache", "/etc/nsswitch.conf", "/etc/passwd", "/etc/group",
                                   "/etc/localtime",   "/etc/resolv.conf",   NULL};
        for (int i = 0; etc_files[i]; i++) {
            struct stat est;
            if (stat(etc_files[i], &est) != 0)
                continue;
            char dst[VAULT_PATH_MAX];
...
#define ENSURE_XDG_DIR(xdg_buf, xdg_buf_sz)                                                                            \
    ...
    if (cfg->iso_audio) {
        ...
        if (!cfg->iso_xdg_runtime) {
            ...
            if (stat(pw_src, &pst) == 0) {
                int fd = open(pw_dst, O_CREAT | O_WRONLY, 0600);
                if (fd >= 0)
                    close(fd);
    ...
    if (cfg->iso_dbus_session && !cfg->iso_xdg_runtime) {
        ...
        if (stat(bus_src, &bst) == 0) {
            int fd = open(bus_dst, O_CREAT | O_WRONLY, 0600);
            if (fd >= 0)
                close(fd);
    ...
    if (cfg->iso_dbus_system) {
        ...
        if (stat(sys_src, &sst) == 0) {
            int fd = open(sys_dst, O_CREAT | O_WRONLY, 0600);
            if (fd >= 0)
                close(fd);
    ...
    if (cfg->iso_dev_level >= 1) {
        ...
        for (int i = 0; devs[i]; i++) {
            ...
            if (fd >= 0)
                ...
                fprintf(stderr, "[RUN] --dev: criar node '%s': %s\n", dst, strerror(errno));
        ...
        char shm_opts[96];
        snprintf(shm_opts, sizeof(shm_opts), "uid=%u,gid=%u,mode=1777,size=256m", (unsigned)real_uid,
                 (unsigned)real_gid);
        if (mount("tmpfs", shm_dst, "tmpfs", MS_NOSUID | MS_NODEV, shm_opts) != 0)
    ...
    /* Firefox: desativa o sandbox INFO do content process
     *
     * O Nuk4sd já dropa TODAS as Linux Capabilities (Layer 4) antes do
     * exec, incluindo CAP_SYS_CHROOT. O Firefox, por padrão, tenta monta
     * seu PRÃ“PRIO sandbox de content-process (chamando chroot()/capset()
     ...
     * minutos, até fechar).
     *
     * Como o Nuk4sd JÃ isola o processo inteiro (user/mount/pid
    ...
    if (gui_mode) {
        ...
        if (cfg->moz_compat) {
            vault_log(LOG_WARN, "[SECURITY] --moz-compat: desativando sandbox interno do Firefox/Mozilla.");
...
#undef ENSURE_XDG_DIR
     ...
     *
     *   [1] umount2(MNT_DETACH): desacopla o /proc herdado sem bloquea
     ...
     *       Monta um procfs NÃO, que enxerga APENAS os PIDs do PID
    ...
    if (!cfg->iso_no_proc && cfg->iso_use_chroot) {
        ...
        if (mount("proc", "/proc", "proc", MS_NOSUID | MS_NOEXEC | MS_NODEV, NULL) != 0) {
            ...
            vault_log(LOG_INFO, "[SANDBOX] /proc remontado (procfs fresco, PID-namespace scoped) "
    ...
    /* /tmp já foi montado como tmpfs antes dos binds; não remontar aqui,
     * pois isso ocultaria os binds de trabalho sob /tmp. */
    ...
    if (cfg->iso_tmp_home) {
        ...
        if (dir) {
             ...
             * processo já Ã‰ o UID real desde o início  não existe mais uma
     ...
     * mais e nunca vai funcionar de novo: dependia de um SEGUNÃO ns-uid
     ...
     * acesso ao FUSE/socket Wayland  o problema que resolvemos ao troca
     ...
     * Os valores explícitos das flags sempre sobrescrevem estes defaults. */
    {
        ...
        long fs = cfg->iso_max_fsize_mb
                   — (long)cfg->iso_max_fsize_mb * 1024 * 1024 : gui_mode
        ...
        if (cfg->iso_max_mem_gb > 0) {
            ...
            rl.rlim_cur = rl.rlim_max = (rlim_t)m;
            setrlimit(RLIMIT_AS, &rl);
            cli_log_rlimit("RLIMIT_AS", m, m);
        }
        rl.rlim_cur = rl.rlim_max = (rlim_t)p;
        setrlimit(RLIMIT_NPROC, &rl);
        cli_log_rlimit("RLIMIT_NPROC", p, p);
        rl.rlim_cur = rl.rlim_max = (rlim_t)fs;
        setrlimit(RLIMIT_FSIZE, &rl);
        cli_log_rlimit("RLIMIT_FSIZE", fs, fs);
        rl.rlim_cur = rl.rlim_max = (rlim_t)fd;
    ...
    if (cfg->iso_no_seccomp) {
        fprintf(stderr, "[RUN] š   --no-seccomp: BPF desativado (modo debug)\n");
        cli_log_seccomp(0);
    } else {
        /* A interface gráfica nunca altera a política de seccomp por conta própria.
         * Qualquer relaxamento exige uma flag que o usuário tenha escolhido e que
         * apareça no comando final: --friendly-sandbox, --permissive-sandbox ou
         * --nested-sandbox. */

        int nested = cfg->nested_sandbox ? 1 : 0;
        vsb_set_seccomp_mode(cfg->seccomp_allow);
    }
    if (permissive) {
        vault_log(LOG_AUDIT,
                  "[SECURITY] --permissive-sandbox ATIVO ‚ exec='%s' ‚ vault_id=%d ‚ pid=%d ‚ "
    ...
    if (nested) {
        vault_log(LOG_AUDIT,
                  "[SECURITY] Modo SANDBOX ANINHADO (nested) ATIVO ‚ exec='%s' ‚ vault_id=%d ‚ "
                  "fronteira externa mantida (kexec/process_vm_writev=KILL); seccomp/namespace/"
                  "setuid do próprio app LIBERADOS  o app monta o sandbox interno dele sem "
                  "conflito com o seccomp do Nuk4sd.",
                  cfg->run_exec ? cfg->run_exec : "—", cfg->vault_id, (int)getpid());
    }

    /* [LANDLOCK] Terceira camada de MAC (restrição VFS) antes do Seccomp */
    if (!(cfg->seccomp_allow & SALLOW_SETUID)) {
        if (vsb_caps_drop(cfg) != 0) {
            vault_log(LOG_ERROR, "Failed to drop capabilities (Layer 4)");
            return -1;
        }
    }
    ...
    static int dispatch(CliConfig * cfg) {
        ...
        /* --new <nome> */
        if (cfg->new_name) {
            /* FIX auditoria: nome vazio ("--new \"\"") chegava ao create sem
             * validação e produzia um vault sem nome. Agora validate_name é
             * consultado na entrada do CLI (mesma regra de rename). */
            if (validate_name(cfg->new_name) != ERR_OK) {
                print_err("Nome de vault inválido (vazio, muito longo ou com caracteres proibidos).");
                ret = 1;
                goto cleanup;
            }
            ...
            if (cfg->protected_vault && !pass) {
                char *entered_password = read_password_silent("Vault password: ");
                strncpy(pass_buf, entered_password, sizeof(pass_buf) - 1);

                char *confirmed_password = read_password_silent("Confirm password: ");
                strncpy(cnf_buf, confirmed_password, sizeof(cnf_buf) - 1);
        ...
        bool needs_id =
            (cfg->op_info || cfg->op_files || cfg->op_status || cfg->op_scan || cfg->op_encrypt || cfg->op_decrypt ||
             cfg->op_resolve || cfg->op_mount || cfg->op_umount || cfg->op_mount_export || cfg->op_export ||
             cfg->op_rm || cfg->op_rename || cfg->op_unlock || cfg->op_passwd || cfg->op_rule || cfg->op_worm_status ||
             cfg->worm_set || cfg->worm_clear || cfg->worm_protected_scan ||
        ...
        char pass_buf[256] = {0};
        char *pass = cfg->password;
        /* --run --no-fuse não lê nem monta o vault: exigir sua senha nesse caso
         * seria apenas um prompt sem efeito. As demais operações preservam a
         * autenticação do vault normalmente. */
        bool needs_pass =
            (cfg->op_encrypt || cfg->op_decrypt || cfg->op_rm || cfg->op_rename || cfg->op_unlock || cfg->op_mount ||
             cfg->op_resolve || cfg->op_export || cfg->op_mount_export || (cfg->run_exec && !cfg->no_fuse));
        ...
        if (cfg->op_info) {
            cli_log_operation_start("INFO", id);
            vault_info_ffi(id);
            goto cleanup;
        }
        if (cfg->op_files) {
            cli_log_operation_start("FILES", id);
            vault_files_ffi(id);
            goto cleanup;
        }
        ...
        if (cfg->op_status) {
            ...
            const char *label = s == 0 — "OK" : s == 1 — "LOCKED" : s == 2 — "ALERT" : "DELETED";
            if (cfg->json_output)
        ...
        if (cfg->op_encrypt) {
            ...
            if (ret == 0)
                print_ok("Files encrypted (AES-256-GCM).");
            else {
                char m[64];
                snprintf(m, sizeof(m), "Encrypt failed (err=%d)", ret);
                print_err(m);
            }
            goto cleanup;
        }
        ...
        if (cfg->op_decrypt) {
            ...
            if (ret == 0)
                print_ok("Files decrypted.");
            else {
                char m[64];
                snprintf(m, sizeof(m), "Decrypt failed (err=%d)", ret);
                print_err(m);
            }
            goto cleanup;
        }
        ...
        if (cfg->op_resolve) {
            ...
            if (ret == 0)
                print_ok("Alert resolved. Status reset to OK.");
            else
                print_err("Resolve failed.");
            goto cleanup;
        }
        ...
        if (cfg->op_mount) {
            ...
            if (ret == 0)
                print_ok("Vault mounted via FUSE.");
            else {
                char m[64];
                snprintf(m, sizeof(m), "Mount failed (err=%d)", ret);
                print_err(m);
            }
            goto cleanup;
        }
        ...
        if (cfg->op_umount) {
            ...
            if (ret == 0)
                print_ok("Vault unmounted.");
            else
                print_err("Unmount failed. If PROTECTED-SCAN, use --mount-export.");
            goto cleanup;
        }
        ...
        if (cfg->op_export || cfg->op_mount_export) {
            ...
            cli_log(CLI_LOG_INFO, "EXPORT", "dest='%s' file='%s'", dst, cfg->export_file ? cfg->export_file : "(all)");
        ...
        if (cfg->op_passwd) {
            ...
            explicit_bzero(cnf_buf, sizeof(cnf_buf));
            if (ret == 0)
                print_ok("Password changed.");
            else
                print_err("Password change failed.");
            goto cleanup;
        }

        if (cfg->op_rule) {
            ret = vault_rule_ffi(id, cfg->rule_max_fails, cfg->rule_hour_from, cfg->rule_hour_to);
            if (ret == 0) {
                char m[128];
                snprintf(m, sizeof(m), "Rule added: max_fails=%d hours=%d-%d", cfg->rule_max_fails, cfg->rule_hour_from,
                         cfg->rule_hour_to);
                print_ok(m);
            } else {
        ...
        if (cfg->op_worm_status) {
            ...
            printf("  %-16s %s\n", "delete:", f & WORM_PROTECT_DELETE ? "\033[31mBLOCKED\033[0m" : "allowed");
            printf("  %-16s %s\n", "rename:", f & WORM_PROTECT_RENAME ? "\033[31mBLOCKED\033[0m" : "allowed");
            printf("  %-16s %s\n", "write:", f & WORM_PROTECT_WRITE ? "\033[31mBLOCKED\033[0m" : "allowed");
            printf("  %-16s %s\n", "read:", f & WORM_PROTECT_READ ? "\033[31mBLOCKED\033[0m" : "allowed");
            printf("  %-16s %s\n\n", "protected-scan:",
                   f & WORM_PROTECT_SCAN ? "\033[31mACTIVE (immutable  use --mount-export to rescue)\033[0m"
                                         : "inactive");
            goto cleanup;
        }
        ...
        if (cfg->worm_protected_scan) {
            ...
            if (fgets(ans, sizeof(ans), stdin) && !strncmp(ans, "yes", 3)) {
                ...
                if (ret == 0)
                    print_ok("PROTECTED-SCAN activated.");
                else
                    print_err("Failed to activate PROTECTED-SCAN.");
            } else {
        ...
        if (cfg->worm_set || cfg->worm_clear)
            goto cleanup;
        ...
        /* --image <alias|url|file:///abs/rootfs.tar.gz>
         * Importa e extrai uma raiz escolhida explicitamente antes do lançamento.
         * Uma falha é terminal: a presença de --image não pode resultar em uma
         * sandbox iniciada silenciosamente com a raiz do vault. */
        if (cfg->image_url) {
            ...
            if (mkdtemp(lowerdir) == NULL) {
                ...
                if (cfg->verbose)
                    printf("   importing image '%s' into %s...\n", cfg->image_url, lowerdir);
                int pull_ret = rust_oci_pull_image(cfg->image_url, lowerdir);
                if (pull_ret != 0) {
                    print_err("Image import failed  sandbox was not started.");
                    rmdir(lowerdir);
                    ret = 1;
                    goto cleanup;
                } else {
        ...
        if (cfg->run_exec) {
            ...
            if (cfg->no_fuse) {
                ...
                /* FIX auditoria: o retorno de vault_mount_ffi era ignorado
                 * sandbox prosseguia mesmo com a montagem FUSE falhada. Agora o
                 * comando aborta com mensagem clara se a montagem não for bem-sucedida. */
                int mount_ret = vault_mount_ffi(id, pass ? pass : "");
                if (mount_ret != 0) {
                    print_err("FUSE mount failed (err=%d) abortando.\n    Verifique /etc/fuse.conf (user_allow_other) "
                              "ou use --no-fuse.");
                    ret = 1;
                    goto cleanup;
                }
                cfg->fuse_mounted = true;
            }
            ...
            if (cfg->verbose) {
                ...
                printf("   net=%s  wayland=%d  x11=%d  ro-home=%d  "
                       ...
                       cfg->iso_no_net ? "isolated" : "host", cfg->iso_wayland, cfg->iso_x11, cfg->iso_ro_home,
                       cfg->iso_no_dbus, cfg->iso_tmp_home, cfg->iso_no_proc, cfg->no_fuse);
            }
            ...
            if (cfg->no_fuse) {
                ...
                /* FIX auditoria: desmontar SOMENTE se a montagem FUSE foi
                 * realmente bem-sucedida (fuse_mounted). Desmontar uma sessão
                 * inexistente pode limpar o mountpoint de outro vault e
                 * mascarar erros. */
                if (cfg->fuse_mounted) {
                    if (cfg->verbose)
    ...
    int vault_cli_parse_and_exec(int argc, char **argv) {
         ...
         * e da função de lançamento isolado (run_isolated).
     ...
     *  CliConfig DIRETO a partir de parmetros tipados  sem montar string de
     *  linha de comando, sem join(" ")/split_whitespace() e sem passar po
     ...
     *
     *  Reaproveita 100% do mesmo caminho de execução (dispatch(),
     ...
     *  usados. Cada array é uma lista de ponteiros C string (sem separado
    ...
    int vault_sandbox_run_ffi(uint32_t vault_id, const char *password, const char *exec_path, bool no_net, bool wayland,
                              bool x11, bool audio, bool ro_home, bool no_fuse, uint32_t seccomp_allow, bool use_chroot,
                              bool debug, const char *const *ro_paths, uint32_t ro_count, const char *const *rw_paths,
                              uint32_t rw_count, const char *const *blacklist_paths, uint32_t blacklist_count) {
        if (!exec_path || !*exec_path)
            return (int)ERR_INVALID_ARGS;

        CliConfig *cfg = calloc(1, sizeof(*cfg));
        if (!cfg)
            return (int)ERR_NO_MEMORY;
        ...
        cfg->run_exec = (char *)exec_path; /* dispatch() apenas lê este caminho */
        ...
        cli_log_set_verbose(cfg->verbose);
        cli_log(CLI_LOG_INFO, "COMMAND", "vault_sandbox_run_ffi: vault=%u exec=%s binds=%d", vault_id, exec_path,
                cfg->bind_count);
