...
enum {
    ...
    OPT_RULE, OPT_HOURS,
    OPT_NEW, OPT_PATH, OPT_PROTECTED,
    /* WORM */
    OPT_WORM_STATUS,
    ...
    OPT_UNSHARE_IPC, OPT_UNSHARE_UTS, OPT_UNSHARE_CGROUP,
...
static void load_profile(CliConfig *cfg, const char *path) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        else if (!strcmp(p, "--unshare-cgroup")) cfg->iso_unshare_cgroup = true;
...
static int parse_flags(int argc, char **argv, CliConfig *cfg) {
    ...
    while ((opt = getopt_long(argc, argv, "qk", long_options, &opt_index)) != -1) {
        ...
        case OPT_RW_HOME:      cfg->iso_rw_home     = true;   break;
        ...
        case OPT_UNSHARE_CGROUP: cfg->iso_unshare_cgroup = true;  break;
        ...
        case OPT_NET: {
            if (strcmp(optarg, "veth") == 0) {
                ...
                /* Defaults */
                if (!cfg->iso_net_veth_ip)   cfg->iso_net_veth_ip   = VETH_IP_JAIL;
                if (!cfg->iso_net_veth_gw)   cfg->iso_net_veth_gw   = VETH_IP_GW;
                if (!cfg->iso_net_veth_name) cfg->iso_net_veth_name = "nuk4sd";
            } else {
        ...
        case OPT_PERMISSIVE_SANDBOX:
            ...
            break;
        case OPT_ALLOW:
            if      (!strcmp(optarg, "socket"))    cfg->seccomp_allow |= SALLOW_SOCKET;
            else if (!strcmp(optarg, "bind"))      cfg->seccomp_allow |= SALLOW_BIND;
            else if (!strcmp(optarg, "listen"))    cfg->seccomp_allow |= SALLOW_LISTEN;
            else if (!strcmp(optarg, "accept"))    cfg->seccomp_allow |= SALLOW_ACCEPT;
            else if (!strcmp(optarg, "connect"))   cfg->seccomp_allow |= SALLOW_CONNECT;
            else if (!strcmp(optarg, "sendto"))    cfg->seccomp_allow |= SALLOW_SENDTO;
            else if (!strcmp(optarg, "recvfrom"))  cfg->seccomp_allow |= SALLOW_RECVFROM;
            else if (!strcmp(optarg, "sendmsg"))   cfg->seccomp_allow |= SALLOW_SENDMSG;
            else if (!strcmp(optarg, "recvmsg"))   cfg->seccomp_allow |= SALLOW_RECVMSG;
            else if (!strcmp(optarg, "socketpair"))cfg->seccomp_allow |= SALLOW_SOCKETPAIR;
            else if (!strcmp(optarg, "getsockopt"))cfg->seccomp_allow |= SALLOW_GETSOCKOPT;
            else if (!strcmp(optarg, "setsockopt"))cfg->seccomp_allow |= SALLOW_SETSOCKOPT;
            else if (!strcmp(optarg, "shmget"))    cfg->seccomp_allow |= SALLOW_SHMGET;
            else if (!strcmp(optarg, "shmat"))     cfg->seccomp_allow |= SALLOW_SHMAT;
            else if (!strcmp(optarg, "shmctl"))    cfg->seccomp_allow |= SALLOW_SHMCTL;
            else if (!strcmp(optarg, "memfd"))     cfg->seccomp_allow |= SALLOW_MEMFD;
            else if (!strcmp(optarg, "clone3"))    cfg->seccomp_allow |= SALLOW_CLONE3;
            else if (!strcmp(optarg, "futex"))     cfg->seccomp_allow |= SALLOW_FUTEX;
            else if (!strcmp(optarg, "fsync"))     cfg->seccomp_allow |= SALLOW_FSYNC;
            else if (!strcmp(optarg, "renameat2")) cfg->seccomp_allow |= SALLOW_RENAMEAT2;
            else if (!strcmp(optarg, "inotify"))   cfg->seccomp_allow |= SALLOW_INOTIFY;
            else if (!strcmp(optarg, "splice"))    cfg->seccomp_allow |= SALLOW_SPLICE;
            else if (!strcmp(optarg, "timerfd"))   cfg->seccomp_allow |= SALLOW_TIMERFD;
            else if (!strcmp(optarg, "signalfd"))  cfg->seccomp_allow |= SALLOW_SIGNALFD;
            else if (!strcmp(optarg, "userfaultfd"))cfg->seccomp_allow |= SALLOW_USERFAULTFD;
            else if (!strcmp(optarg, "ptrace"))    cfg->seccomp_allow |= SALLOW_PTRACE;
            else if (!strcmp(optarg, "bpf"))       cfg->seccomp_allow |= SALLOW_BPF;
            else if (!strcmp(optarg, "mount"))     cfg->seccomp_allow |= SALLOW_MOUNT;
            else if (!strcmp(optarg, "chroot"))    cfg->seccomp_allow |= SALLOW_CHROOT;
            else if (!strcmp(optarg, "setuid"))    cfg->seccomp_allow |= SALLOW_SETUID;
            else {
        ...
        case OPT_NESTED_SANDBOX:     cfg->nested_sandbox = true; break;
        case OPT_MOZ_COMPAT:         cfg->moz_compat = true; break;
...
static int run_isolated(CliConfig *cfg, char *vault_path) {
    ...
    if (cfg->iso_audit) {
        ...
        fprintf(stderr, "[audit] unshare-ipc=%d  unshare-uts=%d  unshare-cgroup=%d  new-session=%d\n",
                cfg->iso_unshare_ipc, cfg->iso_unshare_uts,
                cfg->iso_unshare_cgroup, cfg->iso_new_session);
        for (int i = 0; i < cfg->bind_count; i++) {
            const char *t = cfg->binds[i].type == BINFO       — "ro"
    ...
    if (pid > 0 && cfg->iso_net_veth) {
        if (vsb_net_veth_setup(pid, cfg->iso_net_veth_ip, cfg->iso_net_veth_gw,
                               cfg->iso_net_veth_name) != 0) {
    ...
    if (pid > 0) {
        ...
        if (cfg->iso_unshare_cgroup && cfg->iso_max_mem_gb > 0) {
            snprintf(cgroup_name, sizeof(cgroup_name),
                     "nuk4sd-%ld-%d", (long)getpid(), (int)pid);
            long long memory_mb = (long long)cfg->iso_max_mem_gb * 1024LL;
            if (rust_cgroup_apply(cgroup_name, (unsigned long long)pid,
                                  memory_mb, 1024ULL) != 0) {
    ...
    if (cfg->iso_unshare_cgroup) ns_flags |= CLONE_NEWCGROUP;
#endif
    ...
    if (cfg->iso_net_veth) {
        vsb_configure_veth_inside(cfg->iso_net_veth_ip, cfg->iso_net_veth_gw,
                                  cfg->iso_net_veth_name);
    }
     ...
     * continua confinada ao jail. */
    {
         ...
         */
        char sz_opt[96];
        snprintf(sz_opt, sizeof(sz_opt), "uid=%u,gid=%u,mode=1777,size=%dm",
                 (unsigned)real_uid, (unsigned)real_gid, sz);
                ...
                if (mount(home, dst, NULL, MS_BIND | MS_REC, NULL) == 0) {
                    if (cfg->iso_ro_home) {
                        ...
                        if (cli_mkdir_p(nuk_mnt, 0700) == 0) {
                            ...
                            snprintf(home_tmpfs_opts, sizeof(home_tmpfs_opts),
                                     "uid=%u,gid=%u,mode=0700,size=4m",
                                     (unsigned)real_uid, (unsigned)real_gid);
                            mount("tmpfs", nuk_mnt, "tmpfs",
                                  MS_NOSUID | MS_NODEV, home_tmpfs_opts);
                        }
                    }
                } else {
...
#define ENSURE_XDG_DIR(xdg_buf, xdg_buf_sz)                                    \
    ...
    if (cfg->iso_dev_level >= 1) {
        ...
        char shm_opts[96];
        snprintf(shm_opts, sizeof(shm_opts), "uid=%u,gid=%u,mode=1777,size=256m",
                 (unsigned)real_uid, (unsigned)real_gid);
  ...
  if (cfg->op_whitelist_exclude || cfg->op_whitelist_restore) {
   ...
   if (cfg->run_exec) {
        ...
        if (cfg->no_fuse) {
            ...
            if (mount_ret != 0) {
                print_err("FUSE mount failed (err=%d) abortando.\n    Verifique /etc/fuse.conf (user_allow_other) ou use --no-fuse.");
                ret = 1;
                goto cleanup;
            }
