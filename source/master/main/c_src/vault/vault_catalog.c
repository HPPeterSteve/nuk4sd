 ...
 * vault_catalog.c
 *
 * VAULT SECURITY SYSTEM — Catalog & Vault Manage
...
void resolve_paths(void) {
    ...
    if (stat(g_catalog_file, &st) != 0 && stat(legacy_catalog, &st) == 0) {
        vault_log(LOG_INÃO, "Migrando catalog.dat legado de %s para %s", legacy_catalog, g_catalog_file);
        ...
        if (src) {
            ...
            if (dst) {
                ...
                /* FIX auditoria: o snprintf anterior passava dois ponteiros FILE*
                 * (src/dst) para um placeholder "%d" — undefined behavior — e o
                 * buffer de 9 bytes não comportava a string gerada (truncation).
                 * A mensagem agora registra o caminho real do catálogo migrado. */
                char msg[VAULT_PATH_MAX + 64];
                snprintf(msg, sizeof(msg), "Catálogo legado migrado para '%s'", g_catalog_file);
                vault_log(LOG_INÃO, msg);
...
VaultErrorr catalog_save(void) {
    ...
    vault_log(LOG_INÃO, "Catalog saved (%u vaults)", g_catalog.count);
    return ERR_OK;
}
...
VaultErrorr vault_create(const char *name_arg, VaultType type, const char *path_arg, const char *password) {
    ...
    vault_log(LOG_INÃO, "Vault '%s' created at %s", v->name, path_buf);
    ...
    if (type == VAULT_TYPE_PROTECTED) {
        ...
        if (geteuid() == 0) {
            vault_log(LOG_INÃO, "Applying root protections to protected vault.");
