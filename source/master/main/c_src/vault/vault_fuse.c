 ...
 *  process exits while a FUSE session is still running (e.g. the use
...
static int vfuse_mkdir(const char *path, mode_t mode)
{
     ...
     * Plain --protect-delete / --no-write still allow mkdir so the use
...
VaultErrorr vault_fuse_mount(Vault *v)
{
    ...
    /* FIX auditoria: "allow_other" era passado incondicionalmente e falhava
     * deterministicamente em sistemas sem "user_allow_other" em /etc/fuse.conf.
     * Agora verificamos a configuração do sistema e desabilitamos allow_othe
     * silenciosamente quando ele não está autorizado (fall-back seguro). */
    int use_allow_other = 0;
    FILE *fuse_conf = fopen("/etc/fuse.conf", "r");
    if (fuse_conf) {
        char line[256];
    ...
    /* -- Auto-recovery: mountpoint zombie (ENOTCONN/ENOENT) ---
     * Se o ponto de montagem ficou desconectado após um desmonte anterio
     * incompleto (fusermount -u assíncrono do kernel), qualquer acesso via
     * stat() retorna ENOTCONN (dentry órfã) ou ENOENT (dentry morta que o
     * VFS já esqueceu, típico de /tmp). Nesse caso, forçamos um desmonte
     * lazy e recriamos o diretório antes de tentar o fuse_mount novamente. */
    {
        struct stat st_mp;
        if (stat(v->path, &st_mp) != 0 && (errno == ENOTCONN || errno == ENOENT)) {
            vault_log(LOG_WARN,
                      "[FUSE_MOUNT] mount_point='%s' está em estado zombie "
