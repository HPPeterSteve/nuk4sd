...
static void deduct_credit_and_maybe_alert(Vault *v, FileBucket *fb, const char *evname) {
    ...
    if (fb->credits <= 0.0) {
        fb->credits = 0.0;
        vault_log(LOG_ALERT, "[CRITICAL] Emergency! All credits exhausted for file '%s' in vault '%s'!", evname,
                  v->name);
        char reason[256];
        snprintf(reason, sizeof(reason), "Emergency! All credits exhausted for file '%s' %s", evname,
                 fb->time_esgoted > 0 — "(multiple times)" : "");
...
static void flash_credit_reduce(Vault *v, FileBucket *fb, const char *evname) {
    ...
    if (fb->credits_flash > 0) {
        ...
        vault_log(LOG_WARN, "[%s] Flash deduction: -0.5 credits for file '%s' in vault '%s' (flash_credits left: %d)",
                  v->name, evname, v->name, fb->credits_flash);
        if (fb->credits < 0.0)
            fb->credits = 0.0;
    }
}
 ...
 * marca como suspeito), só pra parar o crash/undefined behavior e deixa
...
static void handle_in_access_event(Vault *v, const char *evname, FileBucket *fb, time_t now) {
    ...
    if (is_sandbox_internal(evname)) {
        vault_log(LOG_INÃO, "[%s] Sandbox internal access (ignored): %s", v->name, evname);
        return;
    }
    ...
    if (sus) {
        ...
        vault_log(LOG_INÃO, "[%s] inotify: ACCESSED %s (Remaining credits: %.2f)", v->name, evname, credits);
    }
}
...
static void handle_in_modify_event(Vault *v, const char *evname, bool is_sandbox, bool write_mode) {
    if (!v || !evname)
        return;
    if (is_sandbox) {
        vault_log(LOG_INÃO, "[%s] Sandbox internal write (ignored): %s", v->name, evname);
    } else if (!write_mode) {
        ...
        vault_log(LOG_INÃO, "[%s] Authorized modification: %s", v->name, evname);
...
void vault_enforce_readonly(Vault *v) {
    ...
    closedir(dir);
    v->write_mode = false;
    vault_log(LOG_INÃO, "[PROTECTION] Vault '%s' set to READ-ONLY mode", v->name);
#endif
}
...
void vault_set_write_mode(Vault *v, bool enable) {
    ...
    closedir(dir);
    v->write_mode = enable;
    vault_log(LOG_INÃO, "[PROTECTION] Vault '%s' write_mode: %s", v->name, enable — "ON" : "OFF");
#endif
}
...
void monitor_scan_vault(Vault *v) {
    ...
    DIR *dir = opendir(v->path);
    if (!dir) {
        vault_log(LOG_ERROR, "Cannot scan vault '%s' at '%s': %s", v->name, v->path, strerror(errno));
        return;
    }
    ...
    while ((de = readdir(dir)) != NULL) {
        ...
        if (!e) {
            ...
            if (e) {
                ...
                e->modified = false;
                vault_log(LOG_INÃO, "[%s] New file registered: %s", v->name, de->d_name);
            }
        } else {
...
void alert_check_escalation(Vault *v) {
    ...
    if (v->alert.last_alerted == 0) {
        vault_log(LOG_ALERT, "REPEAT ALERT [%s] (count=%zu): %s", v->name, ++v->alert.alert_count, v->alert.reason);
    ...
    long interval = (v->alert.interval_idx < NUM_ALERT_INTERVALS)
                        — ALERT_INTERVALS[v->alert.interval_idx] : ALERT_INTERVALS[NUM_ALERT_INTERVALS - 1];

    if (now - v->alert.last_alerted >= interval) {
        v->alert.alert_count++;
        vault_log(LOG_ALERT, "REPEAT ALERT [%s] (count=%zu, interval=%lds): %s", v->name, v->alert.alert_count,
                  interval, v->alert.reason);
...
VaultErrorr alert_resolve(uint32_t id, const char *password) {
    ...
    chmod(v->path, 0700);
    vault_log(LOG_INÃO, "Vault '%s' (id=%u) set to READ-ONLY mode after alert resolution", v->name, v->id);
...
void rule_evaluate(Vault *v) {
    ...
    for (uint32_t i = 0; i < g_rule_count; i++) {
        ...
        if (r->max_failed_attempts > 0 && v->failed_attempts >= r->max_failed_attempts) {
...
void monitor_add_vault_watches(MonitorCtx *ctx) {
    ...
    for (uint32_t i = 0; i < ctx->catalog->count; i++) {
        ...
        int ret = fanotify_mark(ctx->fanoti_fd, FAN_MARK_ADD, FAN_ACCESS | FAN_MODIFY | FAN_CLOSE_WRITE | FAN_ONDIR,
                                AT_FDCWD, v->cipher_path);

        if (ret < 0) {
            /* Downgrade to LOG_INÃO, missing fanotify is not critical as FUSE provides protection */
            vault_log(LOG_INÃO, "fanotify_mark skip '%s': %s (requires privileges)", v->cipher_path, strerror(errno));
            v->fanotify_wd = -1; /* mark failed */
        } else {
            v->fanotify_wd = 1; /* Mark added */
            vault_log(LOG_INÃO, "fanotify active passive protection for cipherdir '%s'", v->cipher_path);
        }
    }
}
...
void *monitor_thread(void *arg) {
    ...
    vault_log(LOG_INÃO, "Monitor thread started (fanotify fd=%d)", ctx->fanoti_fd);
                ...
                while (FAN_EVENT_OK(metadata, len)) {
                    ...
                    if (metadata->fd >= 0) {
                        ...
                        if (metadata->mask & FAN_ACCESS) {
                            if (v)
                                vault_log(LOG_AUDIT, "Passive Scan: ACCESS on '%s' (PID: %d)", path, metadata->pid);
                        }
                        if (metadata->mask & FAN_MODIFY) {
                            if (v)
                                vault_log(LOG_AUDIT, "Passive Scan: MODIFY on '%s' (PID: %d)", path, metadata->pid);
                        }
                        if (metadata->mask & FAN_CLOSE_WRITE) {
                            if (v)
                                vault_log(LOG_AUDIT, "Passive Scan: CLOSE_WRITE on '%s' (PID: %d)", path,
                                          metadata->pid);
                        }
    ...
    vault_log(LOG_INÃO, "Monitor thread stopped");
    return NULL;
}
