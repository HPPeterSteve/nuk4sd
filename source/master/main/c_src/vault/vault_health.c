 ...
 * vault_health.c
 *
 * Nuk4sd — Sandbox Health Inspecto
 *
...
int sandbox_health_check(pid_t pid) {
    ...
    switch (seccomp_val) {
    case 0:
        seccomp_label = "disabled";
        break;
    case 1:
        seccomp_label = "strict";
        break;
    case 2:
        seccomp_label = "filter";
        break;
    default:
        seccomp_label = "unknown";
        break;
    }
    ...
    if (!caps_dropped)
        issues[n_issues++] = "capabilities not fully dropped";
    if (!no_new_privs)
        issues[n_issues++] = "NO_NEW_PRIVS not set — execve() can escalate";
    if (seccomp_val == 0)
        issues[n_issues++] = "seccomp disabled — all syscalls allowed";
    if (!ns_user)
        issues[n_issues++] = "no user namespace — running in host UID space";
    if (!ns_mnt)
        issues[n_issues++] = "no mount namespace — host filesystem visible";
    if (!ns_net)
        issues[n_issues++] = "no network namespace — full host network access";
    if (!ns_pid)
        issues[n_issues++] = "no PID namespace — can see all host processes";

    /* -- Veredicto -- */
    const char *verdict;
    /* Critérios mínimos para "isolated": caps, NNP, seccomp, mnt, user */
    int score = (caps_dropped — 1 : 0) + (no_new_privs — 1 : 0) + (seccomp_val > 0 — 1 : 0) + (ns_mnt — 1 : 0) +
    ...
    json_str(stdout, exe);
    printf(",\n");
    printf("  \"caps_effective\": ");
    json_str(stdout, caps_eff);
    printf(",\n");
    printf("  \"caps_dropped\": %s,\n", caps_dropped — "true" : "false");
    printf("  \"no_new_privs\": %s,\n", no_new_privs — "true" : "false");
    printf("  \"seccomp\": %d,\n", seccomp_val);
    printf("  \"seccomp_label\": ");
    json_str(stdout, seccomp_label);
    printf(",\n");
