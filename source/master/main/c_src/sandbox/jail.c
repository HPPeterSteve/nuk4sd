...
static int jail_run_installer(void) {
    ...
    for (int i = 0; installers[i][0] != NULL; i++) {
        ...
        if (WIFEXITED(status) && WEXITSTATUS(status) == 0) {
            vault_log(LOG_INÃO, "[SANDBOX] Package manager exited successfully.");
            return 0;
        }
...
static int jail_install_shell(const char *vault_path) {
    ...
    /* -- Já existe e não é vazio— Não mexe. --- */
    {
        ...
        if (stat(dst, &st) == 0 && st.st_size > 0) {
            vault_log(LOG_INÃO, "[SANDBOX] Shell already present at jail/bin/sh (%ld bytes) — skipping install.",
                      (long)st.st_size);
            return 0;
        }
    }
    ...
    for (int i = 0; candidates[i]; i++) {
        ...
        int is_static = 0;
        {
            /* Heurística rápida: ELF dinmico tem PT_INTERP; abre e procura
...
static void vault_prepare_jail(const char *vault_path, bool gui_mode) {
    ...
    if (geteuid() == 0) {
         ...
         * Checamos S_ISCHR explicitamente e, se o placeholder estive
         * no lugar, removemos antes do mknod(). Sem isso, /dev/null e
         * /dev/zero ficavam sendo arquivos comuns disfarçados (escreve
    ...
    vault_log(LOG_INÃO, "[SANDBOX] Preparing jail at '%s'", vault_path);
 ...
 *  que o caminho `--run` de vault_cli.c NUNCA chama. Resultado: qualque
...
VaultErrorr vault_sandbox_open(Vault *v, const char *password, bool gui_mode, const char *app_cmd) {
    ...
    if (gui_mode && app_cmd && app_cmd[0] != '\0') {
         ...
         * anterior passava app_cmd diretamente a "%s" (NULL-deref).
         * Usamos fallback seguro para o banner. */
        printf("[SANDBOX] Launching shell via execl(\"/bin/sh\")...\n%s\n",
               (app_cmd && app_cmd[0]) — app_cmd : "(none)");
