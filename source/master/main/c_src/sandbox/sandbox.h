...
#define SBX_LOG(layer, fmt, ...) fprintf(stderr, "\033[36m[SANDBOX][%s]\033[0m " fmt "\n", (layer), ##__VA_ARGS__)
...
#define SBX_OK(layer, fmt, ...) fprintf(stderr, "\033[32m[SANDBOX][OK][%s]\033[0m " fmt "\n", (layer), ##__VA_ARGS__)
...
static inline const char *decode_mount_flags(unsigned long fl, char *buf, size_t sz) {
    ...
    static const struct {
        unsigned long v;
        const char *n;
    } bits[] = {{MS_RDONLY, "RDONLY"},   {MS_NOSUID, "NOSUID"}, {MS_NODEV, "NODEV"}, {MS_NOEXEC, "NOEXEC"},
                {MS_REMOUNT, "REMOUNT"}, {MS_BIND, "BIND"},     {MS_REC, "REC"},     {MS_PRIVATE, "PRIVATE"},
                {MS_SLAVE, "SLAVE"},     {MS_SHARED, "SHARED"}, {MS_MOVE, "MOVE"},   {0, NULL}};
    int first = 1;
...
/*
 *  net.c  Rede isolada (--net veth) e filtro nftables (--nfilter)
 *  */
...
/*
 *  autority.c — Supervisor UUID (--uuid)
 *  */
typedef struct {
    pid_t pid;             /* PID do processo filho/sandbox    */
    char *original_binary; /* caminho absoluto do binário pai  */
    int pipe_fd[2];        /* pipe[0]=read, pipe[1]=write; -1 se não usado */
} UuidArgs;
