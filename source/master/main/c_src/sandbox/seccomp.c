...
static int apply_seccomp_policy(void)
{
    ...
    if (!(g_seccomp_allow & SALLOW_SOCKET))      seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(socket), 0);
    if (!(g_seccomp_allow & SALLOW_BIND))        seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(bind), 0);
    if (!(g_seccomp_allow & SALLOW_LISTEN))      seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(listen), 0);
    ...
    if (!(g_seccomp_allow & SALLOW_CONNECT))     seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(connect), 0);
    if (!(g_seccomp_allow & SALLOW_SENÃO))      seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(sendto), 0);
    if (!(g_seccomp_allow & SALLOW_RECVFROM))    seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(recvfrom), 0);
    if (!(g_seccomp_allow & SALLOW_SENDMSG))     seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(sendmsg), 0);
    if (!(g_seccomp_allow & SALLOW_RECVMSG))     seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(recvmsg), 0);
    if (!(g_seccomp_allow & SALLOW_SOCKETPAIR))  seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(socketpair), 0);
    if (!(g_seccomp_allow & SALLOW_GETSOCKOPT))  seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(getsockopt), 0);
    if (!(g_seccomp_allow & SALLOW_SETSOCKOPT))  seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(setsockopt), 0);
    if (!(g_seccomp_allow & SALLOW_SHMGET))      seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(shmget), 0);
    ...
    if (!(g_seccomp_allow & SALLOW_SHMCTL))      seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(shmctl), 0);
    if (!(g_seccomp_allow & SALLOW_MEMFD))       seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(memfd_create), 0);
    if (!(g_seccomp_allow & SALLOW_CLONE3))      seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(clone3), 0);
    ...
    if (!(g_seccomp_allow & SALLOW_RENAMEAT2))   seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(renameat2), 0);
    ...
    if (!(g_seccomp_allow & SALLOW_USERFAULTFD)) seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(userfaultfd), 0);
    if (!(g_seccomp_allow & SALLOW_PTRACE))      seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(ptrace), 0);
    if (!(g_seccomp_allow & SALLOW_BPF))         seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(bpf), 0);
    ...
    if (!(g_seccomp_allow & SALLOW_CHROOT))      seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), SCMP_SYS(chroot), 0);
