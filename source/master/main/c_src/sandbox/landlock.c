 ...
 *    BINÃO      -> READ_FILE | READ_DIR | EXECUTE
...
int landlock_apply(const CliConfig *cfg, const char *vault_root) {
    ...
    if (abi < 0) {
        vault_log(LOG_INÃO, "[LANDLOCK] kernel sem suporte (ABI < 1) — skipped");
        return -1; /* não fatal */
    }
    ...
    close(ruleset_fd);
    vault_log(LOG_INÃO, "[LANDLOCK] ruleset aplicado com %d bind(s)", cfg->bind_count);
    return 0;
}
