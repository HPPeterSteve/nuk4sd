...
static void sandbox_prepare_mounts(void) {
    ...
    SBX_DBG(L, "\u25b6 mount(proc,/proc,proc,%s): result=%d OK", decode_mount_flags(pfl, fl_buf, sizeof(fl_buf)), rr);
    ...
    char tmp_opts[128];
    snprintf(tmp_opts, sizeof(tmp_opts), "uid=%u,gid=%u,mode=1777,size=64m", (unsigned)geteuid(), (unsigned)getegid());
    int rt = mount("tmpfs", "/tmp", "tmpfs", tfl, tmp_opts);
    SBX_DBG(L, "\u25b6 mount(tmpfs,/tmp,tmpfs,%s,'%s'): result=%d %s", decode_mount_flags(tfl, fl_buf, sizeof(fl_buf)),
    ...
    char shm_opts[128];
    snprintf(shm_opts, sizeof(shm_opts), "uid=%u,gid=%u,mode=1777,size=256m", (unsigned)geteuid(), (unsigned)getegid());
