...
/// Referências `file:///caminho/absoluto/rootfs.tar.gz` são importadas
/// localmente, sem rede. Para referências remotas, tenta usar `skopeo` +
/// `umoci` primeiro e então cai para o modo raw tarball com `reqwest` + `tar`.
pub fn pull_and_extract_image(url: &str, target_dir: &Path) -> Result<(), String> {
    ...
    if let Some(local_path) = url.strip_prefix("file://") {
        if !local_path.starts_with('/') {
            return Err("Local runtime path must be absolute: use file:///absolute/path/rootfs.tar.gz".into());
        }

        let metadata = std::fs::metadata(local_path)
            .map_err(|e| format!("Failed to stat local runtime '{}': {}", local_path, e))?;
        if !metadata.is_file() {
            return Err(format!("Local runtime is not a regular file: {}", local_path));
        }
...
/// Removes a cgroup created by `apply_cgroup_limits` after the child exits.
pub fn remove_cgroup(cgroup_name: &str) -> Result<(), String> {
    let cg = Cgroup::load(hierarchies::auto(), cgroup_name);
    cg.delete().map_err(|e| format!("Failed to remove cgroup: {}", e))
}
