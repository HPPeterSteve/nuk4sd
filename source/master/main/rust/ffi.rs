...
extern "C" {
    ...
    /// Lança o sandbox com CliConfig preenchido direto por parmetros tipados —
...
/// Removes the cgroup after the child exits so the host hierarchy is not leaked.
#[cfg(target_os = "linux")]
#[no_mangle]
pub extern "C" fn rust_cgroup_remove(cgroup_name: *const c_char) -> c_int {
    use crate::oci::remove_cgroup;

    let name = unsafe { CStr::from_ptr(cgroup_name) }.to_string_lossy();
    match remove_cgroup(&name) {
        Ok(()) => 0,
        Err(e) => {
            eprintln!("[CGROUP] cleanup failed: {}", e);
            -1
        }
    }
}
...
/// de bind (ro/rw/blacklist) passadas como parmetros tipados — substitui
