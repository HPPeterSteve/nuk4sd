...
int main(int argc, char **argv) {
    ...
    printf("INFO|abi|sizeof(off_t)=%zu|sizeof(time_t)=%zu|sizeof(void*)=%zu|file_offset_bits=%d\n",
           sizeof(off_t), sizeof(time_t), sizeof(void *), _FILE_OFFSET_BITS);
    ...
    if (path_is_directory(real_base, &st) != 0) return 2;
    ...
    if (access(real_base, R_OK | W_OK | X_OK) != 0) report_errno("access-rwx", errno);
    else report_ok("access-rwx", "read-write-search allowed");

    if (statfs(real_base, &sfs) != 0) report_errno("statfs", errno);
    else {
        printf("INFO|statfs|type=0x%lx|name=%s|bsize=%lu|blocks=%ju|bfree=%ju|bavail=%ju|flags=0x%lx\n",
               (unsigned long)sfs.f_type, fs_name(sfs.f_type), (unsigned long)sfs.f_bsize,
               (uintmax_t)sfs.f_blocks, (uintmax_t)sfs.f_bfree,
               (uintmax_t)sfs.f_bavail, (unsigned long)sfs.f_flags);
        if (sfs.f_bavail == 0) {
    ...
    if (stat(created, &st) != 0) report_errno("created-stat", errno);
    else {
        ...
        if (st.st_uid != uid || st.st_gid != gid) {
            fprintf(stderr, "FAIL|created-owner|expected=%u:%u|actual=%u:%u\n",
                    (unsigned)uid, (unsigned)gid, (unsigned)st.st_uid, (unsigned)st.st_gid);
            fail_count++;
        } else report_ok("created-owner", "matches-effective-uid-gid");
    }
    ...
    if (fd < 0) report_errno("create-file", errno);
    else {
        const char payload[] = "nuk4sd mkdtemp diagnostic\n";
        ssize_t written = write(fd, payload, sizeof(payload) - 1);
        int saved = errno;
        close(fd);
        if (written != (ssize_t)(sizeof(payload) - 1)) report_errno("write-file", saved);
        else report_ok("create-write-file", probe_file);
    }
    ...
    } else report_ok("rename-file", renamed_file);

    if (unlink(renamed_file) != 0 && errno != ENOENT) report_errno("unlink-file", errno);
    else report_ok("unlink-file", "removed");
    if (rmdir(created) != 0) report_errno("rmdir", errno);
    else report_ok("rmdir", "removed");
